...
export async function GET(request: NextRequest) {
    ...
    // OAuth 에러 체크 - invalid_scope 포함
    if (error) {
        console.error('OAuth error:', error, errorDescription);
        
        // 특별히 invalid_scope 에러 처리
        if (error === 'invalid_scope') {
            return NextResponse.redirect(
                new URL(`/auth/error?error=invalid_scope&description=${encodeURIComponent(errorDescription || 'Scope permission denied')}`, request.url)
            );
        }
    ...
    try {
        ...
        const mallId = request.cookies.get('mall_id')?.value || 'dhdshop'; // fallback 추가
        
        // CSRF 방지 - State 검증 (임시로 비활성화)
        /*
